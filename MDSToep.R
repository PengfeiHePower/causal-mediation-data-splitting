...
fdr.mds = length(intersect(S0,S1.mds))/length(S1.mds) 
power.mds = length(intersect(S0, S0.mds))/length(S0.mds) 

fdr.MDS = c(fdr.MDS, fdr.mds)
power.MDS = c(power.MDS, power.mds)

write.csv(fdr.MDS,file=paste('results/fdr/covariance/toep/toep_50_',toepW,toepR*10,'.csv',sep=''),row.names=F)
write.csv(power.MDS, file=paste('results/power/covariance/toep/toep_50_',toepW,toepR*10,'.csv',sep=''), row.names=F)
}
    }
}
