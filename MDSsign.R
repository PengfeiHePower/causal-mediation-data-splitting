...
for(i in 1:length(sign.v)){
    delta = sign.v[i]
    print(delta)
    load(file = paste('data/meanModelSign', delta, '.RData', sep=''))
...
for(i in 1:mds.times){
  ...
  cv.lasso.1 = cv.glmnet(X.full1, Y1, penalty.factor = p.fac1, type.measure = "mse", nfolds = 10)
...
fdr.MDS.Id.1 = c(fdr.MDS.Id.1, fdr.mds)
power.MDS.Id.1 = c(power.MDS.Id.1, power.mds)
write.csv(fdr.MDS.Id.1,file=paste('results/fdr/signal/sign200_',delta,'.csv',sep=''),row.names=F)
write.csv(power.MDS.Id.1, file=paste('results/power/signal/sign200_',delta,'.csv',sep=''), row.names=F)
}
}
