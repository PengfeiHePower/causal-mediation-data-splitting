...
load(file = 'data/dataBlock1.RData')
library(MASS)
library(stats)
library(glmnet)
...
for(reps in 1:50){
  ...
  write.csv(fdr.MDS,file='results/fdr/covariance/block/block100_1.csv',row.names=F)
  write.csv(power.MDS, file='results/power/covariance/block/block100_1.csv', row.names=F)
}
