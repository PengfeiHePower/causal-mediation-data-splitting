...
fdr.mds = length(intersect(S0,S1.mds))/max(1,length(S1.mds)) #0.033
power.mds = length(intersect(S0, S0.mds))/length(S0.mds) #0.955

fdr.MDS.Id.2 = c(fdr.MDS.Id.2, fdr.mds)
power.MDS.Id.2 = c(power.MDS.Id.2, power.mds)
}


save(fdr.MDS.Id.2, power.MDS.Id.2, file = 'results/MDS_Id_2.RData')
