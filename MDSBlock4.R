...
load(file = 'data/dataBlock4.RData')
library(MASS)
library(stats)
library(glmnet)
...
for(reps in 1:50){
  ...
  for(i in 1:mds.times){
    ...
    cv.lasso.1 = cv.glmnet(X.full1, Y1, penalty.factor = p.fac1, type.measure = 'mse', nfolds = 20)
  ...
  fdr.mds = length(intersect(S0,S1.mds))/max(length(S1.mds),1) 
  power.mds = length(intersect(S0, S0.mds))/length(S0.mds) 
  
  fdr.MDS = c(fdr.MDS, fdr.mds)
  power.MDS = c(power.MDS, power.mds)
  
  write.csv(fdr.MDS,file='results/fdr/covariance/block/block100_4.csv',row.names=F)
  write.csv(power.MDS, file='results/power/covariance/block/block100_4.csv', row.names=F)
}
